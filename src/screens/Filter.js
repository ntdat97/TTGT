...
function Filter({ navigation, setFilter, thisState, route }) {
    ...
    const confirm = () => {
        ...
        if (checkClassArray.length === 0 && checkStateArray.length > 0) {
            return firestore()
                .collection('Report').where('statusNumber', 'in', checkStateArray).get()
                .then((querySnapShot) => {
                    const users1 = []
                    querySnapShot.forEach((doc) => {
                        users1.push({
        ...
        if (checkClassArray.length === 0 && checkStateArray.length === 0) {
            return pendingRequest ?
                navigation.navigate('RequestedReport', { nonClassify: true }) :
                navigation.navigate('RequestReport', { nonClassify: true })
        }
        firestore()
            .collection('Report').where('check', 'in', checkClassArray).get()
            .then((querySnapShot) => {
                const users1 = []
                querySnapShot.forEach((doc) => {
                    users1.push({
